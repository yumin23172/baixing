...
class LiebiaoSpider(scrapy.Spider):
    name = 'office_rental_sale'
    allowed_domains = ['beijing.liebiao.com']
    start_urls = ['http://beijing.liebiao.com/xiezilou/']
    ...
    def parse(self, response):
        ...
        for li in li_list:
            ...
            item["size"] = li.xpath(".//div[@class='post-other']/p[2]/text()").extract_first()
            item["contact_url"] = ""
            item["registered_company"] = ""
            item["house_allocation"] = ""
            item["name"] = li.xpath(".//div[@class='detail']/div[2]/span/text()").extract_first().split('：')[-1]
            item["form"] = ""
    ...
    def parse_zhufang_list(self, response):
        ...
        item["floor"] =""
        item["description"] = response.xpath("//div[@class='content-wrap']//text()").extract()
