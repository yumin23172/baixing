...
class LiebiaoSpider(scrapy.Spider):
    ...
    start_urls = ['http://beijing.liebiao.com/zufang/']
    ...
    def parse_zhufang_list(self, response):
        ...
        item["rent_type"] = response.xpath("//div[@class='main-field-wrap clf']/dl[1]/dd/a/text()").extract_first()
        item["pay_method"] = response.xpath("//dd[@class='field-detail price']/span[2]/text()").extract_first()
        item["phone"] = response.xpath("//div[@class='phone-way']//span/text()").extract_first()
        item["floor"] = response.xpath("//div[@class='dec-params clf']/dl[4]/dd").extract_first()
        item["house_allocation"] = response.xpath("//div[@class='post-content']/dl/dd").extract_first()
